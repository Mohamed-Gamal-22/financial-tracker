...
export default function OnboardingFlowModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-3 sm:p-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] lg:ps-[280px]">
      ...
      >
        <p className="text-[11px] sm:text-xs font-extrabold text-sky tracking-wide">طريقة الاستخدام</p>
        <h2
          id={titleId}
          className="mt-1 text-lg sm:text-xl font-extrabold text-text-main tracking-tight"
        >
          {step === 1 ? "ابدأ أولًا بدخل الشهر" : "بعدين سجّل معاملاتك"}
        </h2>
        <p className="mt-2 text-sm font-medium text-text-muted leading-relaxed">
          {step === 1 ? (
            "قبل أي مصروف، لازم تسجّل دخل الشهر (راتب أو أي مصدر دخل). ده الأساس اللي هتمشي عليه الميزانية."
          ) : (
            <>
              <span className="lg:hidden">
                اضغط الزر للانتقال إلى صفحة المعاملات. يمكنك تحديد ميزانية المصروف الشهري (اختياري) من صفحة الميزانية.
              </span>
              <span className="hidden lg:inline">
                من قائمة الجانب اختَر «المعاملات» لإضافة مصروفاتك، أو «الميزانية» لتحديد سقف المصروف الشهري.
              </span>
            </>
          )}
        </p>
        ...
        <ol className="mt-4 sm:mt-5 space-y-2 sm:space-y-2.5">
          ...
          >
            <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-extrabold text-text-inverse">
              1
            </span>
        ...
        {step === 1 ? (
          ...
          <div className="mt-5 sm:mt-6 flex flex-col-reverse sm:flex-row gap-2.5 sm:justify-end">
            <button
              type="button"
              onClick={onDismiss}
              className="w-full sm:w-auto rounded-xl border border-card-border bg-surface px-5 py-2.5 text-sm font-bold text-text-main hover:bg-primary-tint/40 transition-colors cursor-pointer"
            >
              فهمت
            </button>
            <button
              ...
              className="w-full sm:w-auto rounded-xl bg-primary hover:bg-primary-hover text-text-inverse px-5 py-2.5 text-sm font-bold shadow-lg shadow-primary/20 transition-all cursor-pointer"
            >
